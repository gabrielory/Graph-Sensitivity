︠e2a4ff37-522a-4971-97b3-133787cf9c42s︠
attach('../GraphFamily.sage')
attach('../corona_product.sage')
from tabulate import tabulate

# defines a class representing K_{1,m} ⊙ K'_n graphs that inherits from GraphFamily
class CoronaGraphs(GraphFamily):
    def __init__(self):
        super().__init__()
        self.name = "CoronaGraphs - G = K_{1,m}, H = K'_n"
        self.notation = "K_{1,m} ⊙ K'_n "
    ...
    def graph(self, graphDetails):
        ...
        corona = corona_product(star, empty)
        #corona.show()
...
# run the code
graph = CoronaGraphs()
graph.output_results(5,5, True)
#graph.sensitivity_output_table()
︡582f347b-1873-460f-9e58-e451120cc4e2︡{"stdout":"CoronaGraphs - G = K_{1,m}, H = K'_nResults done"}︡{"stdout":"\n"}︡{"done":true}
