...
# defines a class representing K′m ∨nH for H = Cℓ that inherits from TypeIIGeneralizedWindmillGraphFamily
class GenWindmillIIGraphsC(TypeIIGeneralizedWindmillGraphFamily):
    def __init__(self):
        super().__init__()
        self.name = "GenWindmillGraphsType2 - C"
        self.min_l = 3
    ...
    def table_by_formulas(self, max_n, m, l, print_table = False):
        ...
        for n_value in range(self.min_n, max_n + 1):
            ...
            nalphah = n_value*math.floor(l/2)
            alpha = max(m,nalphah)

            if m <= nalphah:

                h_sensitivity = 1 if l != 4 else 2
    ...
    def alternate_table(self, n, m, max_l, min_s = 0, print_table = False):
        ...
        for l_value in range(self.min_l, max_l + 1):
            ...
            h = graphs.CycleGraph(l_value)
            graph = self.graph(gd, h)
            alpha = self.get_alpha(gd)

            if m <= n*math.floor(l_value/2): # m <= n * indenpendence number of C
...
# run code
g = GenWindmillIIGraphsC()
g.output_results(4,4,4)
g.display_lxm(4,4,4)
g.display_lxn(4,4,4)
︡dd30195f-0ec0-4708-8464-0f0db7b852b3︡{"stdout":"GeneralizedWindmillGraphsType2 - C Sensitivity Results lxm done"}︡{"stdout":"\n"}︡{"stdout":"GeneralizedWindmillGraphsType2 - C Sensitivity Results lxn done"}︡{"stdout":"\n"}︡{"done":true}
