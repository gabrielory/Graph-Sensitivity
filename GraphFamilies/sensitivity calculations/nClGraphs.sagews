︠f6fc5674-6a1c-4e57-9dc7-7d7b030b19a5︠
attach('../nHGraphFamily.sage')
from tabulate import tabulate

# defines a class representing nH for H = Cℓ that inherits from nHGraphFamily
class nClGraphs(nHGraphFamily):
    def __init__(self):
        super().__init__()
        self.name = "nClGraphs"
        self.min_l = 3
    ...
    def output_results(self, max_n, max_l, show_eigenvectors = False, lower_n = 5, upper_n = 7, lower_l = 5, upper_l = 7, min_s = 0):
        ...
        for l in range(self.min_l, max_l + 1):
            ...
            t_m = self.table_and_matrices(max_n, l, h, lower_n, upper_n, lower_l, upper_l, min_s)
            t = t_m[0]
    ...
    # write the tables for k-sensitivity to an output file - memory too restrictive to use
...
# run code
g = nClGraphs()
#g.output_results(7,7)
#g.output_ksensitivity(3,5)
︡57b7c640-7225-4b0d-b57d-c7375d6f6ff8︡{"stdout":"nClGraphsKSensitivityTablesDone\n"}︡{"done":true}
