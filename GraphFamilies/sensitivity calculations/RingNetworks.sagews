...
# defines a class representing ring networks that inherits from GraphFamily
class RingNetworks(GraphFamily):
    def __init__(self):
        super().__init__()
        self.name = "RingNetworks"
        self.min_n = 1
        self.min_m = 3

    # returns a ring network with the current values for n and m
    def graph(self, graphDetails):
        ...
        # create  cm
        cycle = graphs.CycleGraph(m)

        # create k'n
        empty = graphs.EmptyGraph()
        empty.add_vertices(range(n))

        ring_network = cycle.lexicographic_product(empty)

        graphDetails.graph = ring_network
        # ring_network.show()
    ...
    def get_eigenvalues(self, min_n = 1, max_n = 10, min_m = 3, max_m = 10, show_eigenvectors = False):
...
# run the code
rn = RingNetworks()
rn.output_results(5,5,True,4,5,4,5)
rn.alternate_display(5,5)
rn.get_eigenvalues()
︡92b346d6-49d2-4ea7-9ba8-ad04309621e5︡{"stdout":"RingNetworksResults done"}︡{"stdout":"\n"}︡{"stdout":"RingNetworksSensitivityResults done"}︡{"stdout":"\n"}︡{"stdout":"RingNetworksEigenvalues done"}︡{"stdout":"\n"}︡{"done":true}
